...
|--------|---------|:----:|-------------|-----|
| [Shibumi Mount Adapter](docs/shibumi-mount-adapter.md) | ![Isometric render of the Shibumi mount capture socket — a rounded-rectangle tray with inward-curling capture lips at the top of the long walls, an open mouth at the near end and a closed arch bridge at the far end, and a low inverted-U tongue of two thin rails on the floor](docs/images/shibumi-mount-adapter/socket-iso.png) | v1 (OpenSCAD) · _fit-test_ | Phase 1 of a beach-chair accessory program: a reverse-engineered **capture socket** — the molded mount that slides onto the chair's cleat. A flat cleat blade (27.6 × 4.02 mm) enters the **open mouth** and slides in lengthwise; inward **capture lips** trap its back face and two thin floor **rails** set its seat; a closed **arch bridge** caps the far end. Dimensions reverse-engineered from caliper photos via a 4-way multi-model pass (in-plane geometry closes on two independent sums). 28.2 × 40.1 × 12.5 mm, watertight, prints support-free stood on its arch end. Ships as a **3-piece clearance ladder** (gap 4.0 / 4.2 / 4.35) to dial the slide-on fit. Later phases add holders (bottle / table / phone) reusing this mount. | [A](designs/shibumi-mount-adapter/output/fit-test/socket-A-nominal-gap4.0.stl) · [B](designs/shibumi-mount-adapter/output/fit-test/socket-B-snug-gap4.2.stl) · [C](designs/shibumi-mount-adapter/output/fit-test/socket-C-sliding-gap4.35.stl) |
| [Mailbox Stand-off Grate](docs/mailbox-standoff-grate.md) | ![Top-down hero of the assembled mailbox grate — four dovetailed quadrant tiles forming one continuous 45-degree diamond cross-hatch mesh, with mostly-solid two-color letters M, H, H, K (one per quadrant) peppered with small drain holes, inside a solid perimeter frame](docs/images/mailbox-standoff-grate/mailbox-standoff-grate-hero.png) | v4 (Fusion + Python) | A raised drainage grate for a **leaky mailbox** — floats mail ~15 mm above pooled water on stand-off legs and drains through the deck. Interior beats the bed *and* the door, so it prints as **four interlocking dovetail-jigsaw quadrants** (2×2, 126.8 × 177.6 mm each) assembled in the box. Top is a **continuous 45° cross-hatch mesh** (~46 % open) with one **two-color letter** per quadrant — **M / H / H / K** — each a mostly-solid glyph with small drain holes (no snag-prone open center, color only the top ~1 mm). Assembled 237.6 × 339.2 × 19 mm, jigsaw interference 0, ~283 g PLA; legs run full-height through the deck so it prints flat, support-free. | [STL](designs/mailbox-standoff-grate/output/assembly-perf.stl) · [3MFs](docs/mailbox-standoff-grate.md#downloads) |
